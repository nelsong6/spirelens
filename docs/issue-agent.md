...
- for SpireLens card-stat tooltip proof, use the MCP route `set_spirelens_view_stats_enabled(true)` -> `show_card_tooltip(surface, card_index)` -> `capture_screenshot` so the screenshot shows the target card tooltip text directly
