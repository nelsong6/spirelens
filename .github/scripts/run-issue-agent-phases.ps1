...
- Keep repository and dependency discovery scoped to this run. Do not search above `$RepoRoot`, under sibling or parent runner workspaces, or inside stale `issue-agent-src` directories from older runs. On the Windows issue-agent runner, the expected STS2 data directory is `D:\Programs\SteamLibrary\steamapps\common\Slay the Spire 2\data_sts2_windows_x86_64`, and the raw game assembly is `D:\Programs\SteamLibrary\steamapps\common\Slay the Spire 2\data_sts2_windows_x86_64\sts2.dll`. When you need game assembly metadata, first use that path, `Sts2PathDiscovery.props`, the configured `Sts2DataDir`, or a focused build in the current checkout to materialize publicized assemblies under the current `$RepoRoot`. Do not start by probing `C:\Program Files`, registry Steam locations, whole-drive scans, or old runner artifacts. If those scoped paths are unavailable, abort with `missing_code_context` instead of scanning `C:\` or reusing old runner artifacts.
...
``````powershell
`$sts2DataDir = "D:\Programs\SteamLibrary\steamapps\common\Slay the Spire 2\data_sts2_windows_x86_64"
