...
- Before writing screenshot or live-validation evidence, call `get_validation_capabilities` and use its returned `card_surfaces`, `relic_surfaces`, `runtime_options`, `recommended_tooltip_evidence_flow`, and `tools[]` manifest as the source of truth for what verification can open, tooltip, screenshot, and mutate. Each referenced tool plan should respect the manifest fields `safe_for_test_planning`, `mutates_state`, `requires_game_running`, `requires_combat`, `output_contract`, `common_failures`, and `examples`. Do not assume an unavailable view exists, and do not omit an available view such as deck, draw_pile, discard_pile, exhaust_pile, relic_select, treasure, or verbose hand stats when it is the right evidence surface.
...
- For relic-stat issues, prefer a deterministic scenario save with the target relic in `relics` or `add_relics`, then require evidence from `get_game_state` showing the resolved relic id/name in the player's relic list plus any visible screenshot surface the validation capabilities support. If the issue changes relic tooltip text and no forced relic-hover MCP tool exists yet, require the strongest available relic-visible screenshot and describe the missing hover capability instead of pretending card tooltip tools can prove relic text.
