...
- `scenario_setup` is the deterministic pre-verification save recipe. Choose the correct `base_save_name` from the verified character identity, such as `base_regent`, `base_ironclad`, `base_silent`, `base_defect`, or `base_necrobinder`. Prefer the smallest complete `deck` of real card ids that satisfies the evidence contract, usually 5 cards when the target card and required support cards can fit in the opening hand. Use a larger deck only when the issue needs draw-pile, discard-pile, exhaust-pile, kill, shuffle, or draw-order evidence, and explain that reason in `scenario_setup.notes`. When a card needs support cards, use `list_cards` with the resolved owner/type/query constraints to select real support card ids from MCP metadata, and set enough energy/max_energy for the planned validation actions. Do not tell verification to use dev-console `fight`/`card` commands for card availability; card availability must come from this save recipe.
