...
function Get-AgentResult {
    ...
    if ([string]::IsNullOrWhiteSpace($result.Path) -or -not (Test-Path -LiteralPath $result.Path)) {
        return $result
    }

    try {
        $raw = Get-Content -LiteralPath $result.Path -Raw -ErrorAction Stop
...
$lines.Add('')
$lines$lines.Add('### Agent Result Contract')
$lines.Add('')
$lines.Add('| Field | Value |')
$lines.Add('| --- | --- |')
...
function Format-ResultValue {
    ...
    if ($Value -is [array]) {
        return (($Value | ForEach-Object { [string]param(
    [string]$EventLogPath,
    [string]$SummaryLogPath,
    [string]$DebugLogPath,
    [string]$ScreenshotDir,
